...
public class HudRenderer {
    ...
    public void render(
        ...
        this.displayedSpeed = Mth.lerp(counter.getGameTimeDeltaPartialTick(true), this.displayedSpeed, snapshot.speedMS());
        ...
        if (ConfigManager.get().extendedHud) {
            ...
            this.renderPing(graphics, i + 75 - nameLen, this.scaledHeight - 65, snapshot.ping());
            ...
            this.typeCentered(graphics, String.format(ANGLE_FORMAT, snapshot.driftAngle()), i, this.scaledHeight - 76, 0xFFFFFFFF);
            this.typeCentered(graphics, String.format(G_FORMAT, snapshot.gForce()), i + 58, this.scaledHeight - 76, 0xFFFFFFFF);

            graphics.text(this.client.font, snapshot.playerName(), i + 88 - nameLen, this.scaledHeight - 65, 0xFFFFFFFF);
        } else {
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, BACKGROUND_COMPACT, i - 91, this.scaledHeight - 83, 182, 20);

            this.renderBar(graphics, i - 91, this.scaledHeight - 83);
            this.typeCentered(graphics, String.format(ConfigManager.get().speedUnit.format, this.displayedSpeed * ConfigManager.get().speedUnit.multiplier), i - 58, this.scaledHeight - 76, 0xFFFFFFFF);
            this.typeCentered(graphics, String.format(ANGLE_FORMAT, snapshot.driftAngle()), i + 58, this.scaledHeight - 76, 0xFFFFFFFF);
        }
    }
